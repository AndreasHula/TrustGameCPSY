...
void shift_updates(True_node** start_node, True_node** end_node, int const& level, double const& monetary_action)	//irritation shift calculation
{
	...
	{
		shifts[level][irr] = (disappointment > 0.0 ? (1.0)*static_cast<double>(irr):(-(1.0)*static_cast<double>(irr))) + shifts[level][irr];
					...
					for(int first_ret = 0; first_ret < nor(first_act); ++first_ret)
					{
						...
						for(int second_act = 0; second_act < noa; ++second_act)
						{
							...
							for(int believed_irr=0; believed_irr < noi; ++believed_irr)
							{
								...
								double disappointment =  expectation -  investor_monetary_action;
								local_shifts[k1][believed_irr] = (disappointment > 0.0 ? 1.0*static_cast<double>(believed_irr):(-1.0*static_cast<double>(believed_irr))) + local_shifts[k1][believed_irr];
...
//-------------------------------------------------------------------------------------------------------------------
	...
	ofstream ofs("/home/hulaa/work/Modelhold/OnlyRisk7.bin", ofstream::out| ofstream::binary);			//output stream
	start_id= 727;
	end_id = 824;
