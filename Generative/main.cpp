...
static True_node* Irritation_Trustee = new True_node();
...
void belief_updates(True_node** start_node, True_node** end_node, int const& level, double const& monetary_action, int const& act, bool const& investor, bool const& investor_model)
{
	...
	if(investor)
	{
		if(investor_model){
			...
			i_exp_payoffs = Irritation_Trustee->get_exp_payoffs(0); 
		}
	}
	else
	{
...
void investor_expectation_calculation( int const& level, True_node** given_pointer, std::array<double, nob> const& running_belief_probability , std::array<double, noi> const& irr_probability, bool const& investor_model)
	...
	if(level > -1)
	{
		...
		if(level == 0) //special case for level 0 //technicality left from an earlier version
		{
			...
			if(investor_model)
			{
				...
				exp_payoffs = Irritation_Trustee -> get_exp_payoffs(0);
			}
			for(int irr=0; irr < noi; ++irr)
			{
				if(irr_probability[irr] > 0.001)
				...
				for(int first_ret = 0; first_ret < nor(money_invested); ++first_ret)
				{
					...
					if(!current_node)
					{
						...
						if((k2+2)%2== 0)
						{
							...
							investor_expectation_calculation( 0, &current_node,  running_belief_probability , running_irr_probability, investor_model);
						}
						else
						{
							for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
							...
							running_irr_probability[0] = 1.0;
							investor_expectation_calculation( -1, &current_node,  running_belief_probability , running_irr_probability, investor_model);
						}
					}
					...
					if(!(money_invested==0))
					{
						...
						{
							belief_updates(&Trustee_node_r, &current_node, k2, trustee_monetary_response , response[game], false, investor_model);
						}
					...
					if(!current_node -> expectation_set(k2))
					{
						if(k2>0)
						{
							...
							investor_expectation_calculation( k2, &current_node,  running_belief_probability , running_irr_probability, investor_model);
						}
						/*else
						{
							for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
					...
					if(investor_model)
					{
						...
						inv_exp_payoffs = Irritation_Trustee -> get_exp_payoffs(0);
					}					
					local_shifts = current_node -> get_shifts();
					for(int irr=0; irr < noi; ++irr)
					{
			...
			//{
				for(int first_ret = 0; first_ret < nor(money_invested); ++first_ret)
				{
					...
					for(int second_act = 0; second_act < noa; ++second_act)
					{
						...
						belief_updates(&current_node, &Trustee_holder, k2+1, investor_monetary_action , action[game+1], true, investor_model);
	...
	else
	{
		...
		if((planning_horizon- game) == 1)
		{
			...
			if(k1>0)
			{
				...
				investor_expectation_calculation( k1-1, &current_node,  running_belief_probability , running_irr_probability, investor_model);
			}
			/*else
			{
				for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
		...
		if((planning_horizon- game) == 2)
		{
			...
			//{
				for(int first_act = 0; first_act < noa; ++first_act)
				{
					...
					if(k1>0)
					{
						belief_updates(&current_node, &Trustee_node_r, k1, investor_monetary_action, action[game], true, investor_model);
					}
					...
					for(int first_ret = 0; first_ret < nor(first_act); ++first_ret)
					{
						...
						if(!investor_holder)
						{
							...
							if((k1+2)%2== 1)
							{
								...
								investor_expectation_calculation( 0, &investor_holder,  running_belief_probability , running_irr_probability, investor_model);
							}
							else
							{
								for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
								...
								running_irr_probability[0] = 1.0;
								investor_expectation_calculation( -1, &investor_holder, running_belief_probability , running_irr_probability, investor_model);
							}
						}
						for(int b=0; b < nob; ++b)
						{
						...
						if(!action[game]==0)
						{
							if(action[game]>1)
							{
								belief_updates(&Trustee_node_r, &investor_holder,  k1+1, trustee_monetary_action, response[game], false, investor_model);
							}
						...
						if(k1 > 0)
						{
							...
							investor_expectation_calculation( k1-1, &investor_holder,  running_belief_probability , running_irr_probability, investor_model);
						}
						/*else
						{
							for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
...
//------------------------------------------------- readout block for prepared behaviour files ------------------------------------------------------------
	ifstream ifile ("/your/GenerativeParameters.bin", ios::in | ios::binary| ios::ate);	//
	...
	static std::array<std::array<int,2>,1000>  Subject_Irritability;
	static std::array<std::array<int,2>,1000>  Subject_Temperature;
	static std::array<std::array<int,2>,1000>   Subject_Risk_Aversion;
	static std::array<std::array<int,2>,1000>   Subject_Irritation_Awareness;
	static std::array<std::array<int,2>,1000>  Subject_Planning;
	static std::array<std::array<int,2>,1000> Subject_Guilt; //store subject guilt values (8100 as arbitrary limit)
	static std::array<std::array<int,2>,1000> Subject_ToM;		//store subject guilt values (8100 as arbitrary limit)						
	for(int sub_id=0; sub_id < no_sub; ++sub_id)
	{
		//for(int turn=0; turn <global_time_horizon; ++turn)
		//{
			Subject_ToM[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_ToM[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];	
			Subject_Guilt[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Guilt[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];	
			Subject_Irritability[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Irritability[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];	
			Subject_Risk_Aversion[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Risk_Aversion[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];	
			Subject_Temperature[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Temperature[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];				
			Subject_Planning[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Planning[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Irritation_Awareness[sub_id][0]= *(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];
			Subject_Irritation_Awareness[sub_id][1]=*(int*)&melblock[il];
			il += sizeof *(int*)&melblock[il];			
		//}
	}
...
//-------------------------------------------------------------------------------------------------------------------
	...
	ofstream ofs("/your/GeneratedGames.bin", ofstream::out| ofstream::binary);			//output stream
	start_id= 0;
	end_id = 824;
	...
	for(int sub_id= start_id; sub_id < end_id; ++sub_id)
	{
		...
		//{
			...
			inv_risk_aversion = risk_grid[Subject_Risk_Aversion[sub_id][0]];
			tru_risk_aversion = risk_grid[Subject_Risk_Aversion[sub_id][1]];

			//for(int temp_count=0; temp_count < 4; ++temp_count)
			//{
				investor_temperature =  temperature_grid[Subject_Temperature[sub_id][0]];
				trustee_temperature = temperature_grid[Subject_Temperature[sub_id][1]];
				...
				if(tru_risk_aversion < 1.2)
				{
					tru_risk_aversion = 1.0;
				...
				//{
					d1 =Planning[Subject_Planning[sub_id][0]]; //get current planning var
					d2 =Planning[Subject_Planning[sub_id][1]];
					...
					//{
						...
						for(int level = 0; level < noT; ++level)
						{
							for(int irr =0; irr < noi; ++irr)
							{
								investor_irr_beliefs[level][irr] = irr_belief_presets[Subject_Irritation_Awareness[sub_id][0]][irr]; //use a preset prior according to irritation belief settings
								trustee_irr_beliefs[level][irr] = irr_belief_presets[Subject_Irritation_Awareness[sub_id][1]][irr];
							}
						}
						...
						for(int time_step=0; time_step < global_time_horizon; ++time_step)	//global_time_horizon
						{
							...
							{			//set up expectation
								...
								{ 		//if -1 investor is part of the ToM hierarchy of the trustee, then beliefs of level 0 trustee on investor need to be accounted for
									...
									investor_expectation_calculation( 0, &trustee_node, main_running_belief_probability , main_irr_probability, false);
								...
								else
								{
									...
									investor_expectation_calculation( -1, &trustee_node,  main_running_belief_probability , main_irr_probability, false);
								}
							}
							if(!investor_node-> expectation_set(0))
							{
								...
								if((Subject_ToM[sub_id][0]+3)%2== 0)
								{
									...
									investor_expectation_calculation( 0, &investor_node, main_running_belief_probability , main_irr_probability, true);
								}
								else
								{
									...
									main_irr_probability[0]= 1.0;
									investor_expectation_calculation( -1, &investor_node,  main_running_belief_probability , main_irr_probability, true);
								}
							}
							...
							for(int first_action = 0; first_action < noa; ++first_action)	//explore all initial investor actions
							{
								...
								{	//if level -1 not part of hierarchy -> all updates (i.e. a level 0 is doing inference on them)
									...
									belief_updates(&investor_node, &Trust, 1, investor_monetary_action , first_action, true, true);
								...
								if((Subject_ToM[sub_id][1]+3)%2== 1 )
								{
									...
									belief_updates(&trustee_node, &Trust, 1, investor_monetary_action, first_action, true, false);
							...
							for(int level = 1; level <= Subject_ToM[sub_id][0]+1 ; ++level) //all initial investor considerations
							{
								...
								{
									//cout << "calculate at " << level << " for " << Subject_ToM[sub_id][0] << endl;
									for(int belief=0; belief < nob ; ++belief)
									{
									...
									if(level <(Subject_ToM[sub_id][0]+1))
									{
										if(!investor_node -> expectation_set(level))
										{
											...
											investor_expectation_calculation( level, &investor_node,  main_running_belief_probability , main_irr_probability, true);
										}
								...
								if((Subject_ToM[sub_id][1]+3-level)%2== 1)
								{
									...
									if(level <(Subject_ToM[sub_id][1]+1))
									{
										if(!trustee_node -> expectation_set(level))
										{
											...
											investor_expectation_calculation( level, &trustee_node,  main_running_belief_probability , main_irr_probability, false);
										}
									}
							...
							//---------------------------------------- Generative Block ----------------------------------------------
							//cout << "Generate!" << endl;
							Subject_Games[sub_id][0][time_step] = softmax( hold_investor_payoffs[Subject_ToM[sub_id][0]][time_step][Subject_Irritability[sub_id][0]][Subject_Guilt[sub_id][0]], 201202);
							ofs.write( reinterpret_cast<char*>( &Subject_Games[sub_id][0][time_step] ), sizeof Subject_Games[sub_id][0][time_step]);
							//cout << "Generated " << Subject_Games[sub_id][0][time_step] << endl;
							//--------------------------------------------------------------------------------------------------------
							...
							for(int level =1; level <= Subject_ToM[sub_id][1]+1 ; ++level)
							{
								...
								if((Subject_ToM[sub_id][1]+3-level)%2== 0)
								{
									...
									belief_updates(&trustee_node, &temp_node, level, investor_monetary_action, Subject_Games[sub_id][0][time_step], true, false);
							...
							if(!(Subject_Games[sub_id][0][time_step]==0))
							{
								Subject_Games[sub_id][1][time_step] = softmax( hold_trustee_payoffs[Subject_ToM[sub_id][1]][time_step][Subject_Irritability[sub_id][1]][Subject_Guilt[sub_id][1]], 201202);
							}
							else
							{
							...
							for(int level = 1; level <=Subject_ToM[sub_id][0]+1 ; ++level)
							{
								...
								if((Subject_ToM[sub_id][0]+3-level)%2== 1)
								{
									...
									belief_updates(&investor_node, &temp_node, level, investor_monetary_action, Subject_Games[sub_id][0][time_step], true, true);
							...
							if(Trust) //save beliefs and shifts outside of the node, so at the beginning of the next round, nodes can be initialized to current values
							{
								...
								{							
									...
									{
										belief_updates(&temp_node, &Trust, Subject_ToM[sub_id][0]+1 , trustee_monetary_response, Subject_Games[sub_id][1][time_step], false, true);
									}
								}									
								investor_belief_parameters = Trust -> get_belief_parameters();
								investor_shifts = Trust -> get_shifts();
